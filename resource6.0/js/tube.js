		...
		if(typeof this._config.cableDistance == "undefined"){
  		...
  		if(typeof _subCables != "undefined"){
  			...
  			for(var i = 0 ; i < _subCables.length ; i ++){  	
  				...
  				_x = this._config.x + this._config.cableDistance;  				
  				if( i == 0 ){  				
  					_y = this._config.y ;
  				}else{  					
  					_y += ((_subCables[i-1].fiberDistance) * (_subCables[i-1].fiberNumPerTube) * (_subCables[i-1].tubeNum)  * (i) + 50 * (_subCables.length - i) + 10 * i);
  				}		
  				_subCables[i].x = _x ;
  				_subCables[i].y = _y ;
		...
		//	console.log("this.addSubTube = function(tubeIndex,subTubeArray,releationships)  end");
  			  		...
  			  		for(var i = 0 ; i < _subTubes.length ; i++){
				  			...
				  			subTube   = tube._config; 			
				  		  	...
				  		  	if( relationship1 != null){
				  			  	...
				  			  	for(var j = 0 ; j < relationship1.relationshipLineProp.length ; j ++){
				  			  		var cableAId = relationship1.relationshipLineProp[j].cableAId;
				  			  		var tubeAIndex  = relationship1.relationshipLineProp[j].tubeAIndex;
				  			  		var _tkey = cableAId + "_" + tubeAIndex;
				  			  		var rela_temp = tempMap.get(_tkey);  
				  			  		if(rela_temp == null){
				  			  			rela_temp = {relationshipLineProp:[]};
				  			  		}
				  			  		rela_temp.relationshipLineProp.push(relationship1.relationshipLineProp[j]);
				  			  		tempMap.put(_tkey,rela_temp);
				  			  	}
				  			  	
				  			  	var tempMapKeys = tempMap.keys();
				  			  	for(var k = 0 ; k < tempMapKeys.length ; k++){
				  			  		var tempMapKey = tempMapKeys[k];
				  			  		var tempMapValue = tempMap.get(tempMapKey);
				  			  		var tubeIndex = tempMapKey.substring(tempMapKey.indexOf("_") + 1,tempMapKey.length);
				  			  		//console.log(" tubeIndex " + tempMapKey );
				  			  		this.addSubTube(tubeIndex,{subTubes:[subTube]},tempMapValue);
  		...
  		this.drawInSvg = function(node){
  			...
  			*/
  			...
  			//计算光纤线的纵坐标的	
  			var fiber_line_space = (( numofnoequals == 0  ?  0 : this._config.cableDistance) - 40 ) / (numofnoequals - 1) ;
  		...
  		//	console.log(fiber_line_space + " : " + fiber_line_space_left_right);
			...
			for(var i = 0 ; i < relationshipLinesPropArray.length ; i ++){			
				 ...
				 * 
				 */
				if(typeof this._config.name != "undefined" && this._config.name != ""){
					var cableText = document.createElementNS(SVGNS,'text');
					resource.util.assginAttribute(cableText,{
				   				id:this._config.cableId + "_text",
				   				x:this._config.x  - (this._config.name.length / 2) * 3,
				   				y:this._config.y - 8,
				   				"font-size":'0.6em',
				   				"font-weight":'normal',
				   				"font-family": 'Verdana,Arial,sans-serif',
				    ...
				    cableText.appendChild(document.createTextNode(this._config.name));
				    document.getElementById("groupText").appendChild(cableText); 			
				}
  		...
  		this.reDraw = function(change,calccuateFiberCoordsFlag,translate,reDrawSubCable){
  			...
  			//计算光纤线的纵坐标
  			var fiber_line_space = (( numofnoequals == 0  ?  0 : (this._config.cableDistance + (oldX - this._config.x))) - 40 ) / (numofnoequals - 1) ;
  			var fiber_line_space_left_right = 40 / 2;  		
  	     	//	console.log(fiber_line_space + " : " + fiber_line_space_left_right);
			...
			for(var i = 0 ; i < relationshipLinesPropArray.length ; i ++){			
				 ...
				 */
				if(typeof this._config.name != "undefined" && this._config.name != ""){
					$("#" + this._config.cableId + "_text").remove();
					var cableText = document.createElementNS(SVGNS,'text');
					resource.util.assginAttribute(cableText,{
				   				id:this._config.cableId + "_text",
				   				x:this._config.x - (this._config.name.length / 2) * 2,
				   				y:this._config.y - 8,
				   				"font-size":'0.6em',
				   				"font-weight":'normal',
				   				"font-family": 'Verdana,Arial,sans-serif',
